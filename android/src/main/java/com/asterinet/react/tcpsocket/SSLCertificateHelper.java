...
final class SSLCertificateHelper {
     ...
     */
    static SSLSocketFactory create(@NonNull final Context context, @NonNull final String rawResourceUri) {
        try (InputStream caInput = getRawResourceStream(context, rawResourceUri)) {
    ...
    private static InputStream getRawResourceStream(@NonNull final Context context, @NonNull final String resourceUri) throws IOException {
        ...
        if (resId == 0) return URI.create(resourceUri).toURL().openStream(); // From metro on development
